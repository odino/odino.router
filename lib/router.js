        ...
        if (result = request.url.match(routeRegex)) {
            ...
            _.each(parameters, function(parameter, index){
                ...
                if (value) {
                    ...
                    var msg = _s.sprintf(
                        'matched route with path "%s", but an empty parameter was detected (%s), this might mean that you mispelled one of your router, concatenating parameters (ie. /:one:two)',
                        route.path,
                        ':' + parameter
                    );
                    throw new Error(msg);
